...
def detect(im, model, transform, device):
    ...
    scores_all, labels_all = probas.max(-1).values, probas.argmax(-1)
    keep = scores_all > 0.7
    bboxes = rescale_bboxes(outputs['pred_boxes'][0], im.size)

    return scores_all[keep].detach(), bboxes[keep].detach(), labels_all[keep].detach()
...
def main(args):
    ...
    # loop over all .jpg/.png in --coco_path
    for img_name in tqdm(os.listdir(args.coco_path), desc="Inference"):
        if not img_name.lower().endswith(('.jpg', '.png')):
            continue
        img_path = os.path.join(args.coco_path, img_name)
        im = Image.open(img_path).convert('RGB')

        # run DETR
        scores, boxes, labels = detect(im, model, transform, device)
        ...
        for score, (xmin, ymin, xmax, ymax), label in zip(
            ...
            # choose class list based on dataset
            class_list = KITTI_CLASSES if args.kitti2coco else COCO_CLASSES
        ...
        # save result
        # construct the new img name like "000193.png" → "000193_inference.png"
        out_path = os.path.join(args.output_dir, img_name.replace('.', '_inference.'))
        cv2.imwrite(out_path, frame)
    total_time = time.time() - start_time
    print('inference time for {} images: {:.2f} seconds'.format(
        len(os.listdir(args.coco_path)), total_time))
